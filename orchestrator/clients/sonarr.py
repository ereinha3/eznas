...
class SonarrClient(ServiceClient):
    ...
    def ensure(self, config: StackConfig) -> EnsureOutcome:
        ...
        stored_api_key = sonarr_secrets.get("api_key")

        if file_api_key:
            api_key = file_api_key
            if stored_api_key != file_api_key:
                sonarr_secrets["api_key"] = file_api_key
                state_dirty = True
        ...
        base_url = service_base_url("sonarr", config, self.INTERNAL_PORT, "/api/v3")
        ...
        try:
            ...
            with ArrAPI(base_url, api_key) as api:
                ...
                # otherwise so Sonarr imports directly.
    ...
    def verify(self, config: StackConfig) -> EnsureOutcome:
        ...
        qb_host = resolve_service_host("qbittorrent", config, caller="sonarr")
        desired_fields = {
            "host": qb_host,
    ...
    def _ensure_root_folder(
        ...
        default_language = self._select_language_profile_id(
            language_profiles, preferred_languages
        )
